...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Du skal lave en karakter for at kunne spille' } })
	end
end)
	...
	for theData, value in pairs(myIdentity) do
		...
		elseif theData == "dateofbirth" then
			if value == "invalid" then
				reason = "Du har ikke en ægte fødseldato!"
				break
			end
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Denne højde er ikke acceptabl!"
					break
				end
			else
				reason = "Denne højde er ikke acceptabl!"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Dit navn er enten for kort eller for langt mindst (2 max 25)'
	end
	...
	if count ~= nameLength then
		return 'Dit navn indeholder bogstaver som ikke er gyldige i det Amerikanske system'
	end
	...
	if spacesInName > 2 then
		return 'Dit navn inholder mere end 2 mellemrum'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Dit navn skal starte med stort'
	end
